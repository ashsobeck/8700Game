...
class Pumpkin():
    ...
    def if_collision(self, snake: list[pygame.Rect]):
        if not self.eaten:
            ...
            self.create_new = False
            # once pumpkin has passed through snake, every collision with snake
            # body will return false
    ...
    def random_pos(self, snake_body: list[list[int, int]], levels: list[list[int ,int]]):
